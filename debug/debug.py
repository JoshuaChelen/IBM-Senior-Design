...
def pipeline():
    ...
    option = int(input("Do you want to use a previous system description? \n"
                       "(0) - No\n"
                       "(1) - Yes\n"))
...
def get_files_in_directory(relative_path: str) -> list[str]:
    ...
    (i.e., the directory where debug.py lives).
    """
    # Project root is the directory containing THIS file (debug.py)
    project_root = Path(__file__).resolve().parent
...
def test_data_generator():
    ...
    choice = int(input())
    queue_network_name = queue_network[choice]
    queue_network_path = (
        project_root / "data" / "queueing-network" / queue_network_name
    ).resolve()

    data_generator.run(queue_network_path)
    input("Press ENTER to continue")
